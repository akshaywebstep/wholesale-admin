...
<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        ...
        <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Add New Variant</h1>
        <p class="text-sm text-slate-500 mt-0.5">Link a new size, color, or custom SKU to an existing product.</p>
    </div>
</div>
...
<div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 max-w-2xl">
    <form action="{{ route('admin.productVariants.store') }}" method="POST">
        ...
        <div class="space-y-5">
            <!-- Product Selection -->
            <div>
                <label class="block text-xs font-medium text-slate-700 uppercase tracking-wider mb-1.5">Select Product <span class="text-red-500">*</span></label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="size" value="{{ old('size') }}" placeholder="e.g. M, L, XL"
                ...
                <div>
                    ...
                    <input type="text" name="color" value="{{ old('color') }}" placeholder="e.g. Navy Blue"
                           class="w-full bg-slate-50/50 border border-slate-200 rounded-xl px-4 py-2.5 text-sm text-slate-800 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="text" name="variant_sku" value="{{ old('variant_sku') }}" required placeholder="e.g. SHIRT-BLU-L"
                ...
                <p class="text-[11px] text-slate-400 mt-1">Unique stock keeping unit identifier for inventory management.</p>
