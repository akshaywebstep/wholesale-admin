...
<div class="max-w-7xl mx-auto space-y-6 pb-12">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 mt-0.5">Manage internal company staff, role permissions, and B2B wholesale
                customer accounts.</p>
        </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            ...
            <div
                class="w-11 h-11 rounded-2xl bg-slate-100 text-slate-700 flex items-center justify-center font-bold text-lg">
                👥
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            ...
            <div
                class="w-11 h-11 rounded-2xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-lg">
                🛡️
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            ...
            <div
                class="w-11 h-11 rounded-2xl bg-emerald-50 text-emerald-600 flex items-center justify-center font-bold text-lg">
                🏢
            </div>
        </div>
        ...
        <div class="bg-white p-5 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
            <div>
                <span class="text-xs font-semibold text-slate-400 uppercase tracking-wider block">Pending
                    Approvals</span>
                <span
                    class="text-2xl font-bold {{ $pendingCount > 0 ? 'text-amber-600' : 'text-slate-900' }} mt-1 block">
                    {{ $pendingCount }} Waiting
                </span>
            </div>
            <div
                class="w-11 h-11 rounded-2xl bg-amber-50 text-amber-600 flex items-center justify-center font-bold text-lg">
                ⏳
            </div>
        </div>
    </div>
    ...
    @if(session('success'))
    <div
        class="p-4 rounded-2xl bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs flex items-center justify-between shadow-sm">
    ...
    @if(session('error'))
    <div
        class="p-4 rounded-2xl bg-red-50 border border-red-200 text-red-800 text-xs flex items-center justify-between shadow-sm">
    ...
    <div class="flex items-center gap-2 border-b border-slate-200/80 pb-px overflow-x-auto">
        <a href="{{ route('admin.users.index', ['tab' => 'all']) }}"
            ...
            <span
                class="px-2 py-0.5 rounded-full text-[10px] font-mono {{ $tab === 'all' ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600' }}">
                {{ $totalCount }}
            </span>
        </a>
        ...
        <a href="{{ route('admin.users.index', ['tab' => 'staff']) }}"
            ...
            <span
                class="px-2 py-0.5 rounded-full text-[10px] font-mono {{ $tab === 'staff' ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600' }}">
                {{ $staffCount }}
            </span>
        </a>
        ...
        <a href="{{ route('admin.users.index', ['tab' => 'customers']) }}"
            ...
            <span
                class="px-2 py-0.5 rounded-full text-[10px] font-mono {{ $tab === 'customers' ? 'bg-blue-100 text-blue-800' : 'bg-slate-100 text-slate-600' }}">
                {{ $customerCount }}
            </span>
        </a>
        ...
        <a href="{{ route('admin.users.index', ['tab' => 'pending']) }}"
            ...
            @if($pendingCount > 0)
            <span
                class="px-2 py-0.5 rounded-full text-[10px] font-mono bg-amber-100 text-amber-800 font-bold animate-pulse">
                {{ $pendingCount }}
            </span>
            @endif
        </a>
    ...
    <div class="bg-white rounded-2xl border border-slate-200/80 shadow-sm overflow-hidden">
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr
                        class="bg-slate-50 border-b border-slate-200/80 text-[11px] font-bold text-slate-500 uppercase tracking-wider">
                ...
                <tbody class="divide-y divide-slate-100 text-xs">
                    ...
                    <tr class="hover:bg-slate-50/70 transition-colors">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-bold text-slate-900 flex items-center gap-1.5">
                                        ...
                                        <span
                                            class="bg-purple-50 text-purple-700 text-[10px] font-bold px-1.5 py-0.2 rounded border border-purple-200">Super</span>
                                        @endif
                                    </div>
                        ...
                        <td class="py-3.5 px-4">
                            ...
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-[11px] font-semibold bg-emerald-50 text-emerald-800 border border-emerald-200">
                                🛒 {{ $user->roles->first()->name ?? 'Wholesale Buyer' }}
                            </span>
                        ...
                        <td class="py-3.5 px-4 text-center">
                            ...
                            <span
                                class="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold bg-amber-50 text-amber-700 border border-amber-200 animate-pulse">
                        ...
                        <td class="py-3.5 px-4 text-right space-x-1 whitespace-nowrap">
                            ...
                            <form action="{{ route('admin.users.approve', $user) }}" method="POST" class="inline-block">
                                @csrf
                                <button type="submit"
                                    class="px-2.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-[11px] transition-colors shadow-sm">
                                    ✓ Approve
                                </button>
                            </form>
                            <form action="{{ route('admin.users.reject', $user) }}" method="POST" class="inline-block">
                                @csrf
                                <button type="submit"
                                    class="px-2.5 py-1.5 rounded-lg bg-red-50 hover:bg-red-100 text-red-700 font-bold text-[11px] transition-colors">
                                    ✗ Reject
                                </button>
                            </form>
                            @endif
