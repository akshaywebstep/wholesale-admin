...
<div class="max-w-3xl mx-auto">
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h1 class="text-2xl font-bold text-slate-900">Edit Warehouse</h1>
            <p class="text-slate-500 text-sm mt-0.5">Update details for warehouse #{{ $warehouse->id }}</p>
        </div>
    ...
    <form id="editWarehouseForm" action="{{ route('admin.warehouses.update', $warehouse) }}" method="POST" class="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-6 space-y-6">
        @csrf
        @method('PUT')

        <div>
            <label for="name" class="block text-sm font-medium text-slate-700 mb-1">Warehouse Name <span class="text-rose-500">*</span></label>
            <input type="text" name="name" id="name" value="{{ old('name', $warehouse->name) }}" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 @error('name') border-rose-500 @enderror">
        ...
        <div>
            ...
            <input type="text" name="location" id="location" value="{{ old('location', $warehouse->location) }}" required placeholder="Start typing address..." class="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 @error('location') border-rose-500 @enderror" autocomplete="off">
        ...
        <div>
            ...
            <select name="status" id="status" required class="w-full px-3.5 py-2.5 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500">
                <option value="active" {{ old('status', $warehouse->status) == 'active' || old('status', $warehouse->status) == '1' ? 'selected' : '' }}>Active</option>
                <option value="inactive" {{ old('status', $warehouse->status) == 'inactive' || old('status', $warehouse->status) == '0' ? 'selected' : '' }}>Inactive</option>
            </select>
        ...
        <div class="pt-4 border-t border-slate-100 flex items-center justify-end gap-3">
            ...
            <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-medium px-5 py-2.5 rounded-xl text-sm transition-all shadow-sm">Update Warehouse</button>
        </div>
    </form>
</div>
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Existing value is already present, so default is true
    let isValidPlaceSelected = true;
    ...
    if (typeof google !== 'undefined' && google.maps && google.maps.places) {
        ...
        // Reset state when user manually types/edits text
