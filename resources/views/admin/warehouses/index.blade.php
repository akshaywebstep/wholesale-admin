...
<div class="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <div>
        <a href="{{ route('admin.warehouses.create') }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-medium px-4 py-2.5 rounded-xl text-sm transition-all shadow-sm active:scale-[0.98]">
...
<div class="bg-white rounded-2xl p-4 border border-slate-200/80 shadow-sm mb-6">
    <form method="GET" action="{{ route('admin.warehouses.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div>
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by name or location..." class="w-full px-3.5 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500">
        </div>
        <div>
            <select name="status" class="w-full px-3.5 py-2 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500">
        ...
        <div class="flex items-center gap-2">
            <button type="submit" class="bg-slate-800 hover:bg-slate-900 text-white text-sm font-medium px-4 py-2 rounded-xl transition-all">Filter</button>
            <a href="{{ route('admin.warehouses.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 text-sm font-medium px-4 py-2 rounded-xl transition-all">Reset</a>
        </div>
    </form>
</div>
                ...
                @forelse($warehouses as $warehouse)
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="py-3.5 px-4 text-right space-x-2">
                            <a href="{{ route('admin.warehouses.show', $warehouse) }}" class="text-slate-500 hover:text-blue-600 font-medium text-xs bg-slate-100 hover:bg-blue-50 px-2.5 py-1.5 rounded-lg transition-colors">View</a>
                            <a href="{{ route('admin.warehouses.edit', $warehouse) }}" class="text-blue-600 hover:text-blue-800 font-medium text-xs bg-blue-50 px-2.5 py-1.5 rounded-lg transition-colors">Edit</a>
                            <form action="{{ route('admin.warehouses.destroy', $warehouse) }}" method="POST" class="inline-block" onsubmit="return confirm('Are you sure you want to delete this warehouse?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="text-rose-600 hover:text-rose-800 font-medium text-xs bg-rose-50 px-2.5 py-1.5 rounded-lg transition-colors">Delete</button>
                            </form>
                        </td>
                    </tr>
                @empty
